...
- Fixed `Failed to delete project: TypeError: fetch failed`. Root cause: the DELETE handler returned `new NextResponse(null, { status: 204 })` (empty 204), which can cause the client `fetch()` to throw `TypeError: fetch failed` in the Next.js dev/Turbopack runtime when the response body is null. Fix: return `NextResponse.json({ ok: true }, { status: 200 })` and update `submitDelete` to treat any non-2xx as an error (no special-case for 204).
